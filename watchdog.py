...
status_pos = secondline.find('status:') + 7
status = str(secondline)[status_pos:(status_pos + 20)]
status = status[1:status.find('<br>')]
watchdog_file.close()
now = datetime.now()
history_copy = ""
...
Status_str = "\r\n<p>Last Check: " + str(time.strftime("%d/%m/%Y %H:%M")) + " Uptime-difference: " \
             + str(uptime_diff.seconds) + " Status: " + status + "</p>"
...
if now.hour == 23 and 48 < now.minute < 53:
...
if 600 < uptime_diff.seconds < 3000 or (uptime_diff.seconds > 7200 and uptime_diff.seconds % 7200 <= 300):
    Status_str = "\r\n<b>Failed, restarting now: " + str(time.strftime("%d/%m/%Y %H:%M")) + " Uptime-difference: "\
                 + str(uptime_diff.seconds) + " Status: " + status + "</b><br>"
else:
    Status_str = "\r\nLast Check: " + str(time.strftime("%d/%m/%Y %H:%M")) + " Uptime-difference: " \
                 + str(uptime_diff.seconds) + " Status: " + status + "<br>" + history_copy
watchdog_file.write(Status_str)
watchdog_file.close()
